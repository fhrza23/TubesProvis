...
# @app.post('/api/ubah_sandi')
# def change_password(request: ChangePasswordRequest, current_user: RegisterUser = Depends(get_current_user)):
#     conn = connect_db()
#     if not conn:
#         raise HTTPException(status_code=500, detail="Failed to connect to database")
    
#     cursor = conn.cursor()
#     cursor.execute('SELECT password FROM users WHERE nik = ?', (current_user[0],))
#     user = cursor.fetchone()

#     if not user or not pwd_context.verify(request.old_password, user[0]):
#         raise HTTPException(status_code=400, detail="Old password is incorrect")
    
#     new_hashed_password = pwd_context.hash(request.new_password)
#     cursor.execute('UPDATE users SET password = ? WHERE nik = ?', (new_hashed_password, current_user[0]))
#     conn.commit()
#     conn.close()
